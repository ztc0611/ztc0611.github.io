...
import { createForest, createUnderstory } from './trees.js?v=20260909-details';
...
function createInlet() {
  ...
  let shrubSeed=90731;
  const shrubRandom=()=>{shrubSeed=(Math.imul(shrubSeed,1664525)+1013904223)|0;return(shrubSeed>>>0)/4294967296;};
  function island(cx,cz,rx,rz,height,seed, trees=true, snow=false) {
    ...
    function exposure(x,z,y){
      ...
      return Math.max(1-smooth(2.2,6.4,y+terrainNoise(x*.3,z*.3)*1.4),smooth(1.05,2.1,slope)*.76);
    }
    for(let i=0;i<pos.count;i++){
      const x=pos.getX(i)+cx,z=pos.getZ(i)+cz,y=elevation(x,z);
      pos.setXYZ(i,x,y,z);
      const n=terrainNoise(x*.8+seed,z*.8);
      const c=rock.clone().lerp(green,smooth(1,7,y)*.7).multiplyScalar(.88+n*.2);
      if(!snow){
        ...
        c.lerp(color('#aa9055'),meadow*.67);
        c.lerp(color('#707672').multiplyScalar(.87+n*.13),stone);
      }
    ...
    if(trees){
      for(let i=0;i<(cz>-80?90:65);i++){
        const centerX=cx+(shrubRandom()*2-1)*rx*.93,centerZ=cz+(shrubRandom()*2-1)*rz*.93;
        if(groundNoise(centerX*.075+seed,centerZ*.075)<.52)continue;
        const spread=2+shrubRandom()*3.5;
        for(let j=0;j<3+Math.floor(shrubRandom()*4);j++){
          const x=centerX+(shrubRandom()*2-1)*spread,z=centerZ+(shrubRandom()*2-1)*spread,y=elevation(x,z);
          const dx=(elevation(x+1,z)-elevation(x-1,z))*.5,dz=(elevation(x,z+1)-elevation(x,z-1))*.5;
          if(y<3.6||y>height*.62||Math.hypot(dx,dz)>.8||exposure(x,z,y)>.42)continue;
          const size=1.1+shrubRandom()*1.5;
          shrubPlacements.push({x,y:y-size*.22,z,size,shade:shrubRandom(),normal:new THREE.Vector3(-dx,1,-dz).normalize()});
        }
      }
