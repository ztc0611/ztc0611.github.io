...
for(const version of ['after']){
 ...
 for(const width of [1000,900,800,710,667,665,650,600,500,390,600,666,668,710,900,1200,1500]){
  ...
  await page.waitForFunction(expected=>window.__renderCheck.width===expected, width,{timeout:20000});
  cameras.push({requestedWidth:width,...await page.evaluate(()=>window.__renderCheck)});
 }
 ...
 if(errors.length||clearedFrames.length||cameras.some(c=>c.lost||c.width!==c.requestedWidth))throw new Error('Resize regression');
