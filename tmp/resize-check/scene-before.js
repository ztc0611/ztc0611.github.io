...
import { createLowClouds } from './clouds.js';
...
function createInlet() {
  ...
  scene.add(createForest(treePlacements));
  ...
  function resize(){const w=mount.clientWidth,h=mount.clientHeight;if(!w||!h)return;renderer.setSize(w,h,false);camera.aspect=w/h;const narrow=w/h<.9;camera.fov=narrow?64:46;cameraBase.set(narrow?8:0,narrow?12:13,narrow?58:44);aim.set(narrow?4:0,narrow?7:8,-80);camera.updateProjectionMatrix();camera.position.copy(cameraBase);camera.lookAt(aim);reflection.setSize(Math.min(1200,Math.round(w)),Math.min(800,Math.round(h)));needsRender=true;}
  ...
  function frame(now){
    ...
    camera.position.copy(cameraBase).add(new THREE.Vector3(pointer.x*.7,pointer.y*.28,0));camera.lookAt(aim.x+pointer.x*.25,aim.y+pointer.y*.1,aim.z);
