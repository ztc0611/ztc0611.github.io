...
export function createDriftwood(placements, waterUniforms, waveCode) {
  ...
  const gray = new THREE.Color('#827c6d'), pale = new THREE.Color('#b6ad98'), tint = new THREE.Color();
  for (let i = 0; i < positions.count; i++) {
    const x = positions.getX(i), y = positions.getY(i), z = positions.getZ(i);
    const angle = Math.atan2(z, x);
    const grain = Math.sin(angle * 7 + y * 2) * .055 + Math.sin(angle * 13 - y * 3) * .027;
    const split = Math.abs(y) > .49 ? Math.sin(angle * 3 + .7) * .015 + Math.cos(angle * 7) * .01 : 0;
    positions.setXYZ(i, x * (1 + grain) + Math.sin((y + .5) * 3) * .13, y + split, z * (1 + grain));
    tint.copy(gray).lerp(pale, .48 + Math.sin(angle * 3 + y) * .22);
    colors.push(tint.r, tint.g, tint.b);
  }
  geometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));
  geometry.computeVertexNormals();
  const grain = surfaceTexture(true);
  const material = new THREE.MeshStandardMaterial({ vertexColors: true, map: grain, bumpMap: grain, bumpScale: .035, roughness: .98 });
  material.onBeforeCompile = shader => {
    shader.uniforms.uTime = waterUniforms.uTime;
    shader.uniforms.uRipples = waterUniforms.uRipples;
    shader.vertexShader = shader.vertexShader.replace('#include <common>', `#include <common>
      ${waveCode}
      attribute vec2 driftwoodAnchor;
      attribute vec2 driftwoodAxis;
      attribute float driftwoodLength;
      vec2 woodDrift(){return vec2(sin(uTime*.013+driftwoodAnchor.x)*.24,cos(uTime*.009+driftwoodAnchor.y)*.18);}
      float woodSlope(){
        vec2 p=driftwoodAnchor+woodDrift(),span=driftwoodAxis*driftwoodLength*.5;
        return (wave(p+span)-wave(p-span))/driftwoodLength;
      }
    `).replace('#include <defaultnormal_vertex>', `#include <defaultnormal_vertex>
      vec3 woodNormal=inverseTransformDirection(transformedNormal,viewMatrix);
      woodNormal.xz-=driftwoodAxis*woodNormal.y*woodSlope();
      transformedNormal=(viewMatrix*vec4(normalize(woodNormal),0.)).xyz;
    `).replace('#include <project_vertex>', `#include <project_vertex>
      vec3 woodWorld=(modelMatrix*instanceMatrix*vec4(transformed,1.)).xyz;
      woodWorld.xz+=woodDrift();
      woodWorld.y+=wave(driftwoodAnchor+woodDrift())+dot(woodWorld.xz-driftwoodAnchor-woodDrift(),driftwoodAxis)*woodSlope();
      mvPosition=viewMatrix*vec4(woodWorld,1.);
      gl_Position=projectionMatrix*mvPosition;
    `);
  };
  material.customProgramCacheKey = () => 'floating-driftwood-v1';
  const pieces = [];
  placements.forEach((log, index) => {
    const center = new THREE.Vector3(log.x, log.y, log.z);
    pieces.push({ center, direction: log.direction, length: log.length, radius: log.radius, log });
    const side = new THREE.Vector3(-log.direction.z, .6, log.direction.x).normalize();
    const branch = center.clone().addScaledVector(log.direction, -log.length * .29).addScaledVector(side, log.radius * .8);
    pieces.push({ center: branch, direction: side, length: log.radius * (index === 0 ? 2.5 : 1.7), radius: log.radius * .4, log });
  });
  // A trunk and its broken branches share one water sample frame, so they move
  // together and follow the same wave function as the surface and its reflection.
  geometry.setAttribute('driftwoodAnchor', new THREE.InstancedBufferAttribute(new Float32Array(pieces.flatMap(p => [p.log.x, p.log.z])), 2));
  geometry.setAttribute('driftwoodAxis', new THREE.InstancedBufferAttribute(new Float32Array(pieces.flatMap(p => [p.log.direction.x, p.log.direction.z])), 2));
  geometry.setAttribute('driftwoodLength', new THREE.InstancedBufferAttribute(new Float32Array(pieces.map(p => p.log.length)), 1));
  const wood = new THREE.InstancedMesh(geometry, material, pieces.length);
  wood.name = 'Floating driftwood';
  const transform = new THREE.Object3D(), up = new THREE.Vector3(0, 1, 0);
  pieces.forEach((piece, index) => {
    transform.position.copy(piece.center);
    transform.quaternion.setFromUnitVectors(up, piece.direction);
    transform.scale.set(piece.radius, piece.length, piece.radius);
    transform.updateMatrix();
    wood.setMatrixAt(index, transform.matrix);
  });
  wood.computeBoundingSphere();
  wood.boundingSphere.radius += .6;
  return wood;
}
